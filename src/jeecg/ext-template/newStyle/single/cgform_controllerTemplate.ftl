...
<#if fileFlag==true>
import org.qihuasoft.web.cgform.entity.upload.CgUploadEntity;
...
public class ${entityName}Controller extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request) {
		try{
			...
			<#list columns as po>
			<#if po.isQuery =='Y'>
			String query_${po.fieldName} = request.getParameter("${po.fieldName}");
			if(query_${po.fieldName}!=null && !"".equals(query_${po.fieldName})){
				
				<#if po.type == "java.lang.String">
				request.setAttribute("${po.fieldName}", query_${po.fieldName});
				detachedCriteria.add(Restrictions.like("${po.fieldName}", "%"+query_${po.fieldName}+"%"));
				<#elseif po.type == "java.util.Date">
				request.setAttribute("${po.fieldName}", new SimpleDateFormat("yyyy-MM-dd").parse(query_${po.fieldName}));
				String startTime = query_${po.fieldName}+" 00:00:00";
				String endTime = query_${po.fieldName}+" 23:59:59";
				...
				<#else>
				request.setAttribute("${po.fieldName}", query_${po.fieldName});
				detachedCriteria.add(Restrictions.eq("${po.fieldName}", query_${po.fieldName}));
				</#if>
			}
			</#if>
	        </#list>
	...
	public void datagrid(${entityName}Entity ${entityName?uncap_first},HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		org.qihuasoft.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, ${entityName?uncap_first}, request.getParameterMap());
		try{
		//自定义追加查询条件
		<#list columns as po>
	...
	public ModelAndView doAdd(${entityName}Entity ${entityName?uncap_first}, HttpServletRequest request) {
		String message = null;
		...
		request.setAttribute("returnURL", "${entityName?uncap_first}Controller.do?list");
	...
	public ModelAndView doUpdate(${entityName}Entity ${entityName?uncap_first}, HttpServletRequest request) {
		String message = null;
