...
import org.qihuasoft.web.system.dao.JeecgDictDao;
...
import org.qihuasoft.web.system.pojo.base.TSLog;
...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	 ...
	 * @return
	 */
	public List<TSType> getTypeList(String typeGroupId) {
		//TSType actType = commonDao.findUniqueByProperty(TSType.class, "typecode", typecode,tsTypegroup.getId());
		List<TSType> ls = commonDao.findHql("from TSType where typegroupid = ?",typeGroupId);
		return ls;
	 ...
	 * @return
	 */
	public String getName(String typecode,String typeGroupId) {
		//TSType actType = commonDao.findUniqueByProperty(TSType.class, "typecode", typecode,tsTypegroup.getId());
		List<TSType> list = commonDao.findHql("select typename from TSType where typecode = ? and typegroupid = ?",typecode,typeGroupId);
		
		if(CollectionUtils.isEmpty(list)){
			return null;
		}
		
		return list.get(0).getTypename();
	}
