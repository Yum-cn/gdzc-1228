...
import org.qihuasoft.core.common.model.common.UploadFile;
...
import org.qihuasoft.core.common.model.json.ValidForm;
...
import org.qihuasoft.core.util.RoletoJson;
import org.qihuasoft.core.util.SetListSort;
...
import org.qihuasoft.tag.vo.datatable.DataTableReturn;
import org.qihuasoft.tag.vo.datatable.DataTables;
...
import org.qihuasoft.web.system.pojo.base.TSUserOrg;
...
public class UserController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(params = "goChangePSW")
	public ModelAndView goChangePSW(HttpServletRequest req) {
		
		return new ModelAndView("main/changePSW");
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "checkOldPSW")
	@ResponseBody
	public AjaxJson checkOldPSW(HttpServletRequest request) {
	...
	public AjaxJson readCardNo(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		List list = systemService.findListbySql("SELECT card_no FROM `guard_card` order by careate_time desc limit 0,1");
	 ...
	 *  
	 */
	@RequestMapping(params = "updatePSW")
	public void updatePSW(HttpServletRequest request) {
		//System.out.println("证明我来过");
		TSUser user = ResourceUtil.getSessionUserName();
		String newpassword = oConvertUtils.getString(request.getParameter("newPSW"));
		//盐值加密
	 ...
	 * @return
	 */
	@RequestMapping(params = "updatePersonalInfo")
	@ResponseBody
	public AjaxJson updatePersonalInfo(TSUser user,HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "个人信息修改成功";
		TSUser u = ResourceUtil.getSessionUserName();
	...
	@RequestMapping(params = "selectUserCheckBox")
	public String selectUserCheckBox(HttpServletRequest request) {
		// 给部门查询条件中的下拉框准备数据
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
		return "system/user/selectUserCheckBoxList";
	}
//	@RequestMapping(params = "selectTeam")
//	public String selectTeam(HttpServletRequest request) {
//		// 任务分解选择项目组
////		List<PrjTeamEntity> prjTeamList = systemService.getList(PrjTeamEntity.class);
////		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(prjTeamList, "prjName", "id"));
//		prjTeamList.clear();
//		System.out.println("测试选择项目组+++++++++++");
//		return "com/pm/rwfjgl/selectPrjTeamList";
//	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "checkStudentNo")
	@ResponseBody
	public AjaxJson checkStudentNo(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		String stuNo = request.getParameter("stuNo");
		List user = userService.findByProperty(TSUser.class, "userName", stuNo);
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        //查询条件组装器
        org.qihuasoft.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			users.setCardNo(user.getCardNo());
...
//				}
//				user.setCardNo(users.getCardNo());
				user.setStatus(Globals.User_Normal);
	...
	public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		org.qihuasoft.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsRole);
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
...
//			}
	...
	public String exportXls(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
		...
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		org.qihuasoft.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsUser, request.getParameterMap());
...
//					Float usernamefloat = Float.parseFloat(tsUser.getUserName());
//					int usernameint = usernamefloat.intValue();
//					System.out.println("修改>>>"+tsUser.getRealName()+"<<>>"+tsUser.getUserName());
//					tsUser.setUserName(String.valueOf(usernameint));
...
//							System.out.println(">>"+usernameint);
						...
						if(flag){
							...
							if(users.size()!=0){
								//用户存在更新
//								System.out.println("修改>>>"+tsUser.getRealName()+"<<>>"+usernameint);
								TSUser user = users.get(0);
								MyBeanUtils.copyBeanNotNull2Bean(tsUser,user);
								user.setDepartid(null);
								systemService.saveOrUpdate(user);
								...
								//不存在则保存
								//TSUser user = users.get(0);
//								System.out.println("新增>>>"+tsUser.getRealName()+"<<>>"+usernameint);
								tsUser.setDepartid(null);
								systemService.save(tsUser);
								for(String roleCode:roles){
									//根据角色编码得到roleid
