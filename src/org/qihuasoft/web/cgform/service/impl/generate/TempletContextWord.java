package org.qihuasoft.web.cgform.service.impl.generate;
...
public class TempletContextWord {
	...
	static{
		PropertiesUtil util = new PropertiesUtil("config/system_config.properties");
	...
	public Template getTemplate(String tableName, String ftlVersion) {
        ...
        try {
			if(CgAutoListConstant.SYS_MODE_DEV.equalsIgnoreCase(_sysMode)){//开发模式
				...
				throw new RuntimeException("config/system_config.properties的freeMarkerMode配置错误：(PUB:生产模式，DEV:开发模式)");
			}
			return template;
		} catch (IOException e) {
			e.printStackTrace();
