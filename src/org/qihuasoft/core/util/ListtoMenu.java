...
import org.qihuasoft.core.enums.SysACEIconEnum;
...
//		menuString.append(icon);
	...
	public static String getHplusMultistageTree(Map<Integer, List<TSFunction>> map,String systemId) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(0);
		int curIndex = 0;
		for (TSFunction function : list) {
			if(function.getTSFunction()!=null && function.getTSFunction().getId().equals(systemId) && !function.getId().equals(systemId)){
				menuString.append("<li class=\"layui-nav-item layui-nav-itemed\">");
				menuString.append("<a href=\"javascript:;\" class=\"ue-clear\"><i class=\"nav-ivon\"></i><span class=\"nav-text\">"+getMutiLang(function.getFunctionName())+"</span></a>");
//				System.out.println(">>>"+getMutiLang(function.getFunctionName()));
				if(!function.hasSubFunction(map)){
					menuString.append("</li>");
					//menuString.append(getSubMenu(function,1,map));
				}else{
					//menuString.append("<b class=\"arrow icon-angle-down\"></b></a><ul  class=\"submenu\" >");
//					menuString.append("<ul class=\"subnav\">");
					menuString.append(getHplusSubMenu(function,1,map));
//					menuString.append("</ul>");
				}
				menuString.append("</li>");
				curIndex++;
			}else if(function!=null && function.getId().equals(systemId) && !function.getId().equals(systemId)){
					menuString.append("<li class=\"layui-nav-item layui-nav-itemed\">");
					menuString.append("<h2><a href=\"javascript:;\" class=\"ue-clear\"><i class=\"h2-icon\"></i><span class=\"nav-text\">"+getMutiLang(function.getFunctionName())+"</span></a></h2>");
					System.out.println(">>>"+getMutiLang(function.getFunctionName()));
					if(!function.hasSubFunction(map)){
						menuString.append("</li>");
						//menuString.append(getSubMenu(function,1,map));
...
//						menuString.append("</ul>");
	...
	public static String getHplusMultistageJsonTree(Map<Integer, List<TSFunction>> map) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(0);
		int curIndex = 0;
		for (TSFunction function : list) {
			menuString.append("{name:\""+getMutiLang(function.getFunctionName())+"\"");
			if(!function.hasSubFunction(map)){
				menuString.append("}");
				//menuString.append(getSubMenu(function,1,map));
			}else{
				//menuString.append("<b class=\"arrow icon-angle-down\"></b></a><ul  class=\"submenu\" >");
				menuString.append("children: [");
				menuString.append(getHplusJsonSubMenu(function,1,map));
				menuString.append("]");
			}
			menuString.append("},");
			curIndex++;
	...
	private static String getLeafOfHplusTree(TSFunction function) {
		...
		if(name!=null && !"".equals(name)){
			menuString.append("<dl class=\"layui-nav-child\">");
			menuString.append("<dd class=\"\"><a href=\""+function.getFunctionUrl()+"\"  target=\"mainFrame\"><i class=\"layui-icon\" style=\"top: 3px;\">&#xe638;</i><cite>"+name+"</cite>");
			menuString.append("</dd>");
			menuString.append("</dl>");
		}
		return menuString.toString();
	}
	
	private static String getHplusJsonSubMenu(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){
					menuString.append(getLeafOfJsonHplusTree(function));
				}
			}
		}
		return menuString.toString();
	...
	private static String getLeafOfJsonHplusTree(TSFunction function) {
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getTSIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
		...
		//addTabs({id:'home',title:'首页',close: false,url: 'loginController.do?home'});
		String name = getMutiLang(function.getFunctionName()) ;
		menuString.append("<li class=\"subnav-li\" href=\""+function.getFunctionUrl()+"\" data-id=\""+function.getId()+"\"><a href=\"javascript:;\" class=\"ue-clear\"><i class=\"subnav-icon\"></i><span class=\"subnav-text\">"+name+"</span></a>");
		menuString.append("</li>");
		return menuString.toString();
	}
}