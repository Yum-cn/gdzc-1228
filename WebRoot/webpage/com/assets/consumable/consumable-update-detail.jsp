...
<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
  ...
  <div class="layui-tab-content">
    <div class="layui-tab-item layui-show">
		<form name="form1" id="form1" method="post" action="consumableController.do?doUpdateDetail">
		<input id="id" name="id" type="hidden" value="${consumableDetailPage.id }">
                    <input id="createName" name="createName" type="hidden" value="${consumableDetailPage.createName }">
                    <input id="createBy" name="createBy" type="hidden" value="${consumableDetailPage.createBy }">
                    <input id="createDate" name="createDate" type="hidden" value="${consumableDetailPage.createDate }">
                    <input id="updateName" name="updateName" type="hidden" value="${consumableDetailPage.updateName }">
                    <input id="updateBy" name="updateBy" type="hidden" value="${consumableDetailPage.updateBy }">
                    <input id="updateDate" name="updateDate" type="hidden" value="${consumableDetailPage.updateDate }">
                    <input id="sysOrgCode" name="sysOrgCode" type="hidden" value="${consumableDetailPage.sysOrgCode }">
                    <input id="sysCompanyCode" name="sysCompanyCode" type="hidden" value="${consumableDetailPage.sysCompanyCode }">
                    <input id="bpmStatus" name="bpmStatus" type="hidden" value="${consumableDetailPage.bpmStatus }">
		<div class="formbody">
		  <ul class="forminfo">
		  	<li>
              <label class="from-label-title">耗材名称:</label>
                <input name="consumableOperateType" id="consumableOperateType" type="hidden" value="use">
                <input name="consumableId" id="relationId" type="hidden" value="${consumableDetailPage.consumableId }">
                <input name="softListNames" class="dfinput" style="vertical-align: top; " value="${consumableDetailPage.name }" id="relationName" datatype="*" />
            ...
            <li>
              <label class="from-label-title">领用时间:</label>
                <input id="useTime" name="useTime" type="text" class="dfinput" class="Wdate" onClick="WdatePicker()" value='<fmt:formatDate value="${consumableDetailPage.useTime }" pattern="yyyy-MM-dd"/>' datatype="*"/>
              <i class="Validform_checktip"></i>
            </li> 
		    <li>
             <label class="from-label-title">数量:</label>
               <input id="number" name="number" value="${consumableDetailPage.number }" type="text" class="dfinput" datatype="/^-?[1-9]+(\.\d+)?$|^-?0(\.\d+)?$|^-?[1-9]+[0-9]*(\.\d+)?$/" nullmsg="不能为空" errormsg="必须为数字，可以有小数"/>
             <i class="Validform_checktip"></i>
             <label class="Validform_label" style="display: none;">数量</label>
            </li>
            <li>
		      <label>所属部门:</label>
		      <input id="departname" name="useOrgCode" type="text" readonly="readonly" class="dfinput" value="${consumableDetailPage.useOrgCode}">
		    ...
		    <li>
              <label class="from-label-title">使用人:</label>
                <input id="useUser" name="usePerson" type="text" class="dfinput" datatype="*" value="${consumableDetailPage.usePerson}"/>
              <i class="Validform_checktip"></i>
            </li>    
		    <li>
		      <label>&nbsp;</label>
