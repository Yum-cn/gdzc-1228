...
<div id="container">
    ...
    <div id="bd">
    	<div id="main">
    	<form name="searchForm" id="searchForm" method="post" action="consumableController.do?detailList" class="layui-form layui-form-pane">
          ...
          <button class="layui-btn" onclick="add('consumableController.do?goAddDetail')"><i class="layui-icon"></i> 耗材领用</button>
<!--           <button class="layui-btn" onclick="deleteNewStyleALLSelect('提示','consumableController.do?doBatchAdd','')"><i class="layui-icon">&#xe640;</i> 批量录入</button>
 -->          <!-- <button class="layui-btn" onclick="add('consumableController.do?goAdds')"><i class="layui-icon"></i> 批量耗材录入</button> -->
          ...
          <button class="layui-btn" onclick="add('storeController.do?exportXls')"><i class="layui-icon"></i> 导出</button>
           -->
          <form name="searchForm" id="searchForm" method="post" action="consumableController.do?detailList" >
            <div class="table">
					  ...
					  <table class="layui-table">
					    ...
					    <c:forEach items="${resultList}" var="resultList" varStatus="stu">
					      <tr>
									...
									<td><t:listDictParse parseId="${resultList.consumableOperateType}" style="1" typecode="consumeOpType"></t:listDictParse></td>	
									<td><t:listDictParse parseId="${resultList.useOrgCode}" style="1" typecode="sysOrgCode"></t:listDictParse></td>
									<c:if test="${resultList.consumableOperateType=='use'}">
									<td>${resultList.usePerson}</td>
                                    <td><fmt:formatDate value="${resultList.useTime}" pattern="yyyy-MM-dd"/></td>
									</c:if>
									<c:if test="${resultList.consumableOperateType=='add'}">
                                   <td>${resultList.addPerson}</td>
                                    <td><fmt:formatDate value="${resultList.addTime}" pattern="yyyy-MM-dd"/></td>
                                    </c:if>
									
									<td>${resultList.number}</td>
									<td><a href="javascript:void(0);" onclick="openWin('${resultList.id}');" class="tablelink">打印</a>
                            <a href="#" onclick="add('consumableController.do?exportXls&id=${resultList.id}')" class="tablelink">导出</a>
                            <a href="consumableController.do?goUpdateDetail&id=${resultList.id}" class="tablelink">修改</a>
                            <a href="javascript:void(0);" onclick="deleteDialog('提示','consumableController.do?doDelDetail','${resultList.id}')" class="tablelink" > 删除</a>
                            </td>
					       <%--  <td>
				  ...
				  %>
				    ...
				    <ul class="paginList">
				    			<%
										if(currentPage>1){
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('consumableController.do?detailList&str_currentPage=<%=currentPage-1 %>')"  style="width:76px;text-decoration: none;"><span class="pagepre"> < 上一页</span></a></li>
								<%
										}else{
								%>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('consumableController.do?detailList&str_currentPage=<%=i+1 %>')"><%=i+1 %></a></li>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('consumableController.do?detailList&str_currentPage=<%=currentPage+1 %>')"  style="width:76px;text-decoration: none;"><span class="pagenxt">下一页 > </span></a></li>
								<%
										}else{
								%>
...
//      }
	...
	function openWin(id){
        var addurl="consumableController.do?print&id="+id;
        $.dialog({
            content: 'url:'+addurl,
            zIndex: 10000,
            lock : true,
