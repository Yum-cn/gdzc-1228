...
<div id="container">
    ...
    <div id="bd">
    	<div id="main">
    	<form name="searchForm" id="searchForm" method="post" action="scrapController.do?list" class="layui-form layui-form-pane">
    	  <div class="layui-form-item">
		    <label class="layui-form-label">报废时间:</label>
		    <div class="layui-input-inline">
				<input id="scrapTime" name="scrapTime" type="text"  onClick="WdatePicker()" value="<fmt:formatDate value="${scrapTime}" pattern="yyyy-MM-dd"/>" placeholder="请选择日期" autocomplete="off" class="layui-input"/>
		    </div>
		    <label class="layui-form-label">报废原因:</label>
		    <div class="layui-input-inline">
				<input id="reason" name="reason" type="text" value="${reason}" placeholder="请输入报废原因" autocomplete="off" class="layui-input"/>
		    </div>
		    <div class="layui-input-inline">
          ...
          <button class="layui-btn" onclick="add('scrapController.do?goAdd')"><i class="layui-icon"></i> 报废登记</button>
          <button class="layui-btn" onclick="addExport('scrapController.do?exportXls')"><i class="layui-icon"></i> 导出</button>
          <button class="layui-btn" onclick="deleteNewStyleALLSelect('提示','scrapController.do?doBatchDel','')"><i class="layui-icon">&#xe640;</i> 批量删除</button>
          <form name="searchForm" id="searchForm" method="post" action="scrapController.do?list" >
            <div class="table">
					  ...
					  <table class="layui-table">
					    ...
					    <c:forEach items="${resultList}" var="resultList">
					      <tr>
									...
									<td><fmt:formatDate value="${resultList.scrapTime}" pattern="yyyy-MM-dd"/></td>
									<td>${resultList.reason}</td>


					        <td>
					        <a href="javascript:void(0);" onclick="openWin('${resultList.id}');" class="tablelink">打印</a>
					        <a href="scrapController.do?goUpdate&id=${resultList.id}" class="tablelink">修改</a> 
					         <a href="javascript:void(0);" onclick="deleteDialog('提示','scrapController.do?doDel','${resultList.id}')" class="tablelink" > 删除</a></td>
					      </tr>
					      </c:forEach>
				  ...
				  %>
				    ...
				    <ul class="paginList">
				    			<%
										if(currentPage>1){
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('scrapController.do?list&str_currentPage=<%=currentPage-1 %>')"  style="width:76px;text-decoration: none;"><span class="pagepre"> < 上一页</span></a></li>
								<%
										}else{
								%>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('scrapController.do?list&str_currentPage=<%=i+1 %>')"><%=i+1 %></a></li>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('scrapController.do?list&str_currentPage=<%=currentPage+1 %>')"  style="width:76px;text-decoration: none;"><span class="pagenxt">下一页 > </span></a></li>
								<%
										}else{
								%>
...
<script type="text/javascript">
	function add(url){
	   ...
	   function addExport(url){
	        ...
	        var paramStr="";
	        if(scrapTime){
	            paramStr += "&scrapTime="+$("#scrapTime").val();
	        }
	        if(reason){
	            paramStr += "&reason="+$("#reason").val();
	        }
	        url=encodeURI(encodeURI(url+paramStr));
	...
	//导入
	function ImportXls() {
		openuploadwin('Excel导入', 'scrapController.do?upload', "scrapList");
	}
	
	//导出
	function ExportXls() {
		JeecgExcelExport("scrapController.do?exportXls","scrapList");
	}
	   function openWin(id){
	        var addurl="scrapController.do?print&id="+id;
	        $.dialog({
	            content: 'url:'+addurl,
	            zIndex: 10000,
	            lock : true,
