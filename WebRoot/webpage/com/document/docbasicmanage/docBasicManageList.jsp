...
<div id="container">
    ...
    <div id="bd">
    	<div id="main">
    	<form name="searchForm" id="searchForm" method="post" action="docBasicManageController.do?list" class="layui-form layui-form-pane">
          ...
          <button class="layui-btn" onclick="add('docBasicManageController.do?goAdd')"><i class="layui-icon"></i> 添加</button>
          <button class="layui-btn" onclick="deleteNewStyleALLSelect('提示','docBasicManageController.do?doBatchDel','')"><i class="layui-icon">&#xe640;</i> 批量删除</button>
          <form name="searchForm" id="searchForm" method="post" action="docBasicManageController.do?list" >
            <div class="table">
					  ...
					  <table class="layui-table">
					    ...
					    <c:forEach items="${resultList}" var="resultList">
					      <tr>
									...
									<td>${resultList.indexList}</td>

									<td>${resultList.smtpService}</td>

									<td>${resultList.senderAddress}</td>

									<td>${resultList.senderEmailPsd}</td>

					        <td><a href="docBasicManageController.do?goUpdate&id=${resultList.id}" class="tablelink">修改</a>  <a href="javascript:void(0);" onclick="deleteDialog('提示','docBasicManageController.do?doDel','${resultList.id}')" class="tablelink" > 删除</a></td>
					      </tr>
					      </c:forEach>
				  ...
				  %>
				    ...
				    <ul class="paginList">
				    			<%
										if(currentPage>1){
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docBasicManageController.do?list&str_currentPage=<%=currentPage-1 %>')"  style="width:76px;text-decoration: none;"><span class="pagepre"> < 上一页</span></a></li>
								<%
										}else{
								%>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docBasicManageController.do?list&str_currentPage=<%=i+1 %>')"><%=i+1 %></a></li>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docBasicManageController.do?list&str_currentPage=<%=currentPage+1 %>')"  style="width:76px;text-decoration: none;"><span class="pagenxt">下一页 > </span></a></li>
								<%
										}else{
								%>
...
<script type="text/javascript">
	...
	//导入
	function ImportXls() {
		openuploadwin('Excel导入', 'docBasicManageController.do?upload', "docBasicManageList");
	}
	
	//导出
	function ExportXls() {
		JeecgExcelExport("docBasicManageController.do?exportXls","docBasicManageList");
	}
</script>
</body>
</html>
