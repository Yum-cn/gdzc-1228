...
<div id="container">
    ...
    <div id="bd">
    	<div id="main">
    	<form name="searchForm" id="searchForm" method="post" action="docLibraryListController.do?list" class="layui-form layui-form-pane">
          ...
          <button class="layui-btn" onclick="createPackage('${pId}')"><i class="layui-icon"></i>新建文件夹</button>
          <button class="layui-btn" onclick="deleteNewStyleALLSelect('提示','docLibraryListController.do?doBatchDel','')"><i class="layui-icon">&#xe640;</i> 批量删除</button>
          <form name="searchForm" id="searchForm" method="post" action="docLibraryListController.do?list" >
            <div class="table">
                <div class="grid">
					  <table class="layui-table">
					    ...
					    <c:forEach items="${resultList}" var="resultList">
					      <tr>
									...
									<td>${resultList.folderName}</td>
					        <td><a href="docLibraryListController.do?goUpdate&id=${resultList.id}" class="tablelink">修改</a>  <a href="javascript:void(0);" onclick="deleteDialog('提示','docLibraryListController.do?doDel','${resultList.id}')" class="tablelink" > 删除</a></td>
					      </tr>
					      </c:forEach>
				  ...
				  %>
				    ...
				    <ul class="paginList">
				    			<%
										if(currentPage>1){
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docLibraryListController.do?list&str_currentPage=<%=currentPage-1 %>')"  style="width:76px;text-decoration: none;"><span class="pagepre"> < 上一页</span></a></li>
								<%
										}else{
								%>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docLibraryListController.do?list&str_currentPage=<%=i+1 %>')"><%=i+1 %></a></li>
								...
								%>
								<li class="paginItem"><a href="javascript:void(0);" onclick="onSearchForm('docLibraryListController.do?list&str_currentPage=<%=currentPage+1 %>')"  style="width:76px;text-decoration: none;"><span class="pagenxt">下一页 > </span></a></li>
								<%
										}else{
								%>
...
<script type="text/javascript">
	...
	//导入
	function ImportXls() {
		openuploadwin('Excel导入', 'docLibraryListController.do?upload', "docLibraryListList");
	}
	
	//导出
	function ExportXls() {
		JeecgExcelExport("docLibraryListController.do?exportXls","docLibraryListList");
	}
</script>
</body>
</html>
		...
		if(pId!=null&&pId!=""){
			layer.prompt({
				  ...
				  function(value, index, elem){
						$.post(
								"docLibraryListController.do?createPackage",
								{pId:pId,pName:value},
								function(res){
