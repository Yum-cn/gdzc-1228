...
<html>
 ...
 <body>
  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="conclusionController.do?doAdd" tiptype="1" >
		...
		<tr>
						...
						<td class="value">
							...
							<input id="fruitMainFileDocName" name="fruitMainFileDocName" type="hidden" style="width: 150px" class="inputxt">
											<c:if test="${applyTopicPage.fruitMainFileDoc=='' || applyTopicPage.fruitMainFileDoc==null}">
												<a   target="_blank" id="main_href">暂时未上传</a><input class="ui-button" type="button" value="上传成果主件" onclick="uploadDoc(uploadMainCallBack);"/>
											</c:if>
											<c:if test="${applyTopicPage.fruitMainFileDoc!='' && applyTopicPage.fruitMainFileDoc!=null}">
												<a href="${applyTopicPage.fruitMainFileDoc}" target="_blank" id="main_href">${applyTopicPage.fruitMainFileDocName}</a>
												<input class="ui-button" type="button" value="重新上传成果主件" onclick="uploadDoc(uploadMainCallBack);"/>
											</c:if>							
			...
			<table width="100%" cellpadding="0" cellspacing="1" class="formtable" id="content">
			  <tr style="height:30px">
			    ...
			    <td align="center" class="Validform_label" valign="middle"><input type="button" name="button" value="新增成果" onclick="uploadDoc(uploadTemplateCallBack);"/></td>
			  </tr>
			  <c:forEach items="${conclusionList }" var="conclusionList" varStatus="stu">
			  <tr style="height:30px">
			    <td class="value" align="center">&nbsp;1</td>
			    <td class="value" align="center">&nbsp;<input id="name" name="name" type="text" style="width: 96%" class="inputxt" value="${conclusionList.name}"></td>
			    ...
			    <select name="shape" id="shape">
		          <option value="论文" ${conclusionList.shape=='论文'?'selected':'' }>论文</option>
		          <option value="教育叙事" ${conclusionList.shape=='教育叙事'?'selected':'' }>教育叙事</option>
		          <option value="随笔" ${conclusionList.shape=='随笔'?'selected':'' }>随笔</option>
		          <option value="教学设计" ${conclusionList.shape=='教学设计'?'selected':'' }>教学设计</option>
		          <option value="教学课件" ${conclusionList.shape=='教学课件'?'selected':'' }>教学课件</option>
		          <option value="教育案例" ${conclusionList.shape=='教育案例'?'selected':'' }>教育案例</option>
		          <option value="调研报告" ${conclusionList.shape=='调研报告'?'selected':'' }>调研报告</option>
		          <option value="教具制作" ${conclusionList.shape=='教具制作'?'selected':'' }>教具制作</option>
		          <option value="其他" ${conclusionList.shape=='其他'?'selected':'' }>其他</option>
		        </select>
		        </td>
			    <td class="value" align="center">&nbsp;<a href="${conclusionList.filePath }">${conclusionList.fileName }</a></td>
			    <td class="value" align="center">&nbsp;<input type="button" name="button" value="删除" onclick="remove_line(this);"/></td>
			  </tr>
			  </c:forEach>
			</table>
		</t:formvalid>
